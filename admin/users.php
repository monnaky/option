...
async function loadUsers(page = 1, search = '') {
    try {
        ...
        const url = `${apiBase}/admin.php?action=users&page=${page}&limit=20${search ? '&search=' + encodeURIComponent(search) : ''}`;
...
async function suspendUser(userId) {
    ...
    try {
        const apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
        await apiCall(`${apiBase}/admin.php?action=user-suspend`, 'POST', {
            user_id: userId
        });
...
async function activateUser(userId) {
    ...
    try {
        const apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
        await apiCall(`${apiBase}/admin.php?action=user-activate`, 'POST', {
            user_id: userId
        });
...
async function deleteUser(userId) {
    ...
    try {
        const apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
        await apiCall(`${apiBase}/admin.php?action=user-delete`, 'POST', {
            user_id: userId
        });
